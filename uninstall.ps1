...
# `throw` (not `exit`): this script is normally run via `irm ... | iex`, which
# executes it in the *current* PowerShell host's scope rather than a child
# process/scriptblock. `exit` in that context terminates the whole host
# process -- including, when run inside a Herdr pane, the pane's shell, which
# closes the entire tab. `throw` unwinds only to the try/catch below.
function Die($msg) { throw $msg }

# Everything that can fail or be cancelled lives in this function so that
# both `Die` and the "declined confirmation" path can use `throw`/`return`
# to stop early without ever calling `exit` (see the comment on `Die` above).
function Invoke-BaaTonUninstall {
