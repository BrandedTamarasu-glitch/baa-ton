...
export async function handleActivation(configDir, event, api) {
  ...
  try {
    ...
    if (!info.isFile() || info.isSymbolicLink() || (info.mode & 0o022)) throw new Error('Unsafe activation journal');
    journal = JSON.parse(await readFile(path, 'utf8'));
  } catch (error) { if (error.code === 'ENOENT') return undefined; throw error; }
  if (journal.version !== 1 || journal.operation !== 'reload-pi-runtime' || journal.status !== 'pending') return undefined;
  if (event.data?.pane_id !== journal.paneId || event.data?.workspace_id !== journal.workspaceId || event.data?.agent !== 'pi' ||
    event.data?.agent_status !== 'idle') return undefined;
  ...
  if (agent?.agent !== 'pi' || agent?.pane_id !== journal.paneId || agent?.workspace_id !== journal.workspaceId ||
    agent?.agent_status !== 'idle' || agent?.agent_session?.kind !== 'path' || agent.agent_session.value !== journal.sessionPath)
    return { accepted: true, activation: 'identity-or-readiness-mismatch' };
  const lock = `${path}.lock`;
  try { await mkdir(lock, { mode: 0o700 }); }
  catch (error) { if (error.code === 'EEXIST') return { accepted: true, activation: 'already-claimed' }; throw error; }
  try {
    const current = JSON.parse(await readFile(path, 'utf8'));
    if (current.id !== journal.id || current.status !== 'pending') return { accepted: true, activation: 'already-claimed' };
    ...
    try { await api.request('agent.prompt', { target: journal.paneId, text: '/reload' }); }
    catch { return { accepted: true, activation: 'submission-uncertain', id: journal.id }; }
